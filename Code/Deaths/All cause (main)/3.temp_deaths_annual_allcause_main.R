...
class(data_deaths_allcause_main_cts$date) # check formatting - it is date
...
class(data_deaths_allcause_main_aggr$date) # check formatting - it is date
...
head(data_deaths_allcause_main_cts) # check data frame
...
data_deaths_allcause_main_cts <- data_deaths_allcause_main_cts %>% # Join data with 'date'
  left_join(dailypm25) # Joining with `by = join_by(date)

# Create additional columns with the lag temperatures (previous days)
data_deaths_allcause_main_cts[, paste("tmean", 1:14, sep="_"):=data.table::shift(tasmean, 1:14),
                              by=LSOA21CD] # create lag variables - 1-14 days, creates extra columns

# Create 7 day temperature  average (for step preceding DLNM)
data_deaths_allcause_main_cts[,tasmeanavg7:= data.table::frollmean(tasmean,n = 7,align = "right", adaptive = FALSE, na.rm = TRUE),
                              by = LSOA21CD]
data_deaths_allcause_main_cts[is.na(tasmeanavg7), tasmeanavg7 := tasmean] # days 1-6 in dataset use daily temperature
...
# Join by date
data_deaths_allcause_main_aggr <- data_deaths_allcause_main_aggr %>%
  left_join(daily_temp_aggr) # Joining with `by = join_by(date)

# Add/merge Pm2.5 to aggr dataset
data_deaths_allcause_main_aggr <- data_deaths_allcause_main_aggr %>% # Join data with 'date'
  left_join(dailypm25) # Joining with `by = join_by(date)

# Step 3b: Create lagged environmental series in the aggregated dataset

# Create additional columns
data_deaths_allcause_main_aggr[, paste("tmean", 1:14, sep="_"):=data.table::shift(tasmean, 1:14)] # create lag variables - 1-14 days, creates extra columns

# Create 7 day temperature  average for aggregate dataset - for simple 7 day moving average
data_deaths_allcause_main_aggr[,tasmeanavg7:= data.table::frollmean(tasmean,n = 7,align = "right", adaptive = FALSE, na.rm = TRUE),]
data_deaths_allcause_main_aggr[is.na(tasmeanavg7),tasmeanavg7 := tasmean] # for the first 6 rows, use tasmean
