...
data_deaths_allcause_main_aggr$d074 <- data_deaths_allcause_main_aggr$total-data_deaths_allcause_main_aggr$d75plus
data_deaths_allcause_main_aggr_tempgroup$d074 <- data_deaths_allcause_main_aggr_tempgroup$total-data_deaths_allcause_main_aggr_tempgroup$d75plus

# Fit quasi-poisson regression model (quasi-poisson for overdispersion - variance > mean)
qpmodel074a <- glm(d074 ~ spline,data_deaths_allcause_main_aggr,family=quasipoisson)
summary(qpmodel074a)
...
ggplot(data_deaths_allcause_main_aggr, aes(x=date)) +
...
# Save the plot
ggsave("Outputs/Deaths/All cause/Main/2. Quasi-poisson model (before DLNM)/fig19_splinemodel_seasonality_trends074_main.svg", width=10, height=5)

# Plot response residuals from model (observed - expected)
residuals074a <- residuals(qpmodel074a,type="response")
ggplot(data_deaths_allcause_main_aggr, aes(x=date,residuals074a)) +
...
# Save the plot
ggsave("Outputs/Deaths/All cause/Main/2. Quasi-poisson model (before DLNM)/fig20_resvar074a_main.svg", width=10, height=5)

# Fit second quasi-poisson regression model and include dow
qpmodel074b <- glm(d074 ~ spline + factor(dow), data_deaths_allcause_main_aggr, family=quasipoisson)
summary(qpmodel074b)
...
# Save the plot
ggsave("Outputs/Deaths/All cause/Main/2. Quasi-poisson model (before DLNM)/fig21_spline+dow074_main.svg", width=10, height=5)
...
# Save the plot
ggsave("Outputs/Deaths/All cause/Main/2. Quasi-poisson model (before DLNM)/fig22_resvardow2074_main.svg", width=10, height=5)
...
unadjmodeltgd074 <- glm(d074 ~ temp_group, data_deaths_allcause_main_aggr_tempgroup,family = quasipoisson())
...
adjmodeltgd074a <- glm(d074 ~ temp_group + spline,data_deaths_allcause_main_aggr_tempgroup,family = quasipoisson())
...
adjmodeltgd074b <- glm(d074 ~ temp_group + spline + dow, data_deaths_allcause_main_aggr_tempgroup, family = quasipoisson())
...
adjmodeltgd074c <- glm(d074 ~ temp_group + spline + dow + pm25, data_deaths_allcause_main_aggr_tempgroup, family = quasipoisson())
...
ggsave("Outputs/Deaths/All cause/Main/2. Quasi-poisson model (before DLNM)/fig23_forestplotqpregress74_allcausedeathsmain.svg", width=10, height=5)
...
# Fit quasi-poisson regression model (quasi-poisson for overdispersion - variance > mean)
qpmodel75a <- glm(d75plus ~ spline,data_deaths_allcause_main_aggr,family=quasipoisson)
summary(qpmodel75a)
...
# Save the plot
ggsave("Outputs/Deaths/All cause/Main/2. Quasi-poisson model (before DLNM)/fig24_splinemodel_seasonality_trends75_main.svg", width=10, height=5)

# Plot response residuals from model (observed - expected)
residuals75a <- residuals(qpmodel75a,type="response")
ggplot(data_deaths_allcause_main_aggr, aes(x=date,residuals75a)) +
  geom_point(colour = "black",size = 0.6) +
  geom_hline(yintercept = 0, # 0 is the reference
             linetype = "dashed",
             linewidth = 0.7) +
...
# Save the plot
ggsave("Outputs/Deaths/All cause/Main/2. Quasi-poisson model (before DLNM)/fig25_resvar75a_main.svg", width=10, height=5)

# Fit second quasi-poisson regression model and include dow
qpmodel75b <- glm(d75plus ~ spline + factor(dow), data_deaths_allcause_main_aggr, family=quasipoisson)
summary(qpmodel75b)
...
# Save the plot
ggsave("Outputs/Deaths/All cause/Main/2. Quasi-poisson model (before DLNM)/fig26_spline+dow75_main.svg", width=10, height=5)
...
# Save the plot
ggsave("Outputs/Deaths/All cause/Main/2. Quasi-poisson model (before DLNM)/fig27_resvardow275_main.svg", width=10, height=5)

# 5.2 Exposure/temperature as a categorical variable - unadjusted and adjusted model

# Unadjusted model - only temperature (categorical) and d75 deaths
unadjmodeltgd75 <- glm(d75plus ~ temp_group, data_deaths_allcause_main_aggr_tempgroup,family = quasipoisson())
...
# Adjusted model (1) to include spline (seasonality and long term trends)
adjmodeltgd75a <- glm(d75plus ~ temp_group + spline,data_deaths_allcause_main_aggr_tempgroup,family = quasipoisson())
...
adjmodeltgd75b <- glm(d75plus ~ temp_group + spline + dow, data_deaths_allcause_main_aggr_tempgroup, family = quasipoisson())
...
adjmodeltgd75c <- glm(d75plus ~ temp_group + spline + dow + pm25, data_deaths_allcause_main_aggr_tempgroup, family = quasipoisson())
...
ggsave("Outputs/Deaths/All cause/Main/2. Quasi-poisson model (before DLNM)/fig28_forestplotqpregress75_allcausedeathsmain.svg", width=10, height=5)
