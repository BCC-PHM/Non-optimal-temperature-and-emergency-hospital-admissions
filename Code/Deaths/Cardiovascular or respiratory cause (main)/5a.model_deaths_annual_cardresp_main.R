...
data_deaths_cardresp_main_aggr$dayseq <- seq(nrow(data_deaths_cardresp_main_aggr))
spline <- ns(data_deaths_cardresp_main_aggr$dayseq,df=59)

# Fit quasi-poisson regression model (quasi-poisson for overdispersion - variance > mean)
qpmodel1 <- glm(total ~ spline,data_deaths_cardresp_main_aggr,family=quasipoisson)
summary(qpmodel1)
...
# Save the plot
ggsave("Outputs/Deaths/Cardresp cause/Main/2. Quasi-poisson model (before DLNM)/fig12_splinemodel_seasonality_trends_main.svg", width=10, height=5)
...
# Save the plot
ggsave("Outputs/Deaths/Cardresp cause/Main/2. Quasi-poisson model (before DLNM)/fig13_resvar1_main.svg", width=10, height=5)
...
data_deaths_cardresp_main_aggr$dow <- as.factor(data_deaths_cardresp_main_aggr$dow)

# Fit second quasi-poisson regression model and include dow
qpmodel2 <- glm(total ~ spline+factor(dow),data_deaths_cardresp_main_aggr,family=quasipoisson)
summary(qpmodel2)
...
# Save the plot
ggsave("Outputs/Deaths/Cardresp cause/Main/2. Quasi-poisson model (before DLNM)/fig14_spline+dow_main.svg", width=10, height=5)
...
# Save the plot
ggsave("Outputs/Deaths/Cardresp cause/Main/2. Quasi-poisson model (before DLNM)/fig15_resvardow2_main.svg", width=10, height=5)
...
data_deaths_cardresp_main_aggr_tempgroup <- data_deaths_cardresp_main_aggr %>%
...
data_deaths_cardresp_main_aggr_tempgroup$temp_group <- relevel(data_deaths_cardresp_main_aggr_tempgroup$temp_group, ref = "15-19°C") # Apply relevel function

# Unadjusted model - only temperature (categorical) and total deaths
unadjmodeltg_deaths <- glm(total ~ temp_group, data_deaths_cardresp_main_aggr_tempgroup,family = quasipoisson())
...
adjmodeltg_deaths1 <- glm(total ~ temp_group + spline,data_deaths_cardresp_main_aggr_tempgroup,family = quasipoisson())
...
adjmodeltg_deaths2 <- glm(
  ...
  data_deaths_cardresp_main_aggr_tempgroup,
  family = quasipoisson())
...
ggsave("Outputs/Deaths/Cardresp cause/Main/2. Quasi-poisson model (before DLNM)/fig16_forestplotqpregresstotal_main.svg", width=10, height=5)
...
devresdeathsplot <- plot(data_deaths_cardresp_main_aggr_tempgroup$date,devresdeaths,ylim=c(-5,10),pch=19,cex=0.7,col=grey(0.6),
                         main="Deviance residuals from main model over time",ylab="Deviance residuals",xlab="Date")
abline(h=0,lty=2,lwd=2)

# Save/print plot
dev.print(svg, file="Outputs/Deaths/Cardresp cause/Main/2. Quasi-poisson model (before DLNM)/fig17_devrianceesidualsplot_main.svg", width=10, height=5)
...
# Save/print plot
dev.print(svg, file="Outputs/Deaths/Cardresp cause/Main/2. Quasi-poisson model (before DLNM)/fig18_partialautocorrelationplot_main.svg", width=10, height=5)
