...
splinedayofyear <- onebasis(data_deaths_cardresp_main_cts$doy, "ns", df=3) # to capture within year seasonal trends
...
cb_temp <- list(fun="ns", knots=quantile(data_deaths_cardresp_main_cts$tasmean, c(10,50,90)/100, na.rm=TRUE))
cb_lag <- list(fun="ns", df=5)
group <- factor(paste(data_deaths_cardresp_main_cts$LSOA21CD, data_deaths_cardresp_main_cts$period, sep="-"))
cbtmean <- crossbasis(data_deaths_cardresp_main_cts$tasmean, lag=14, argvar=cb_temp, arglag=cb_lag, group=group)

summary(cbtmean) # summary of cross-basis

# Define the strata - LSOA, year, month 
data_deaths_cardresp_main_cts[, stratum:=factor(paste(LSOA21CD, year, month, sep=":"))] # to compare days within the same LSOA and same month and year

# Run the model, including empty strata (otherwise bias in gnm with quasipoisson)
data_deaths_cardresp_main_cts[,  keep:=sum(total)>0, by=stratum]
...
cbtmeanaggr <- crossbasis(data_deaths_cardresp_main_aggr$tasmean, lag=14, argvar=cb_temp, arglag=cb_lag,
                          group=data_deaths_cardresp_main_aggr$period)
...
# Save the plot
dev.print(svg, file="Outputs/Deaths/Cardresp cause/Main/3. Main model (DLNM)/fig29_cardrespdeaths_main_ctsaggr_plot.svg", width=7, height=4)
...
head(data_deaths_cardresp_main_cts) # check data frame
...
cbint1 <- cbtmean * (data_deaths_cardresp_main_cts$imdscore - intval[1])
...
cbint2<- cbtmean * (data_deaths_cardresp_main_cts$imdscore - intval[2])
...
# Save the plot
dev.print(svg, file="Outputs/Deaths/Cardresp cause/Main/3. Main model (DLNM)/fig30_bhamimdplotcardrespdeaths.svg", width=7, height=4)
...
cbint3 <- cbtmean * (data_deaths_cardresp_main_cts$indoorssubscore - intvalsubdomain[1])
...
# Define interaction cross-bases and create percentiles (high)
cbint4 <- cbtmean * (data_deaths_cardresp_main_cts$indoorssubscore - intvalsubdomain[2])
...
# p = 0.695 which means it there is no statistically significant difference between temperature and deaths and temperature x IMD and deaths. IMD not having an impact.
...
# Save the plot
dev.print(svg, file="Outputs/Deaths/Cardresp cause/Main/3. Main model (DLNM)/fig31_bhamsimdsubdomainplotcardrespdeaths.svg", width=7, height=4)
