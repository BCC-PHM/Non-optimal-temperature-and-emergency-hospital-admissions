...
splinedayofyear <- onebasis(data_adm_cardresp_sen_cts$doy, "ns", df=3) # to capture within year seasonal trends
...
cb_temp <- list(fun="ns", knots=quantile(data_adm_cardresp_sen_cts$tasmean, c(10,50,90)/100, na.rm=TRUE))
cb_lag <- list(fun="ns", df=5)
group <- factor(paste(data_adm_cardresp_sen_cts$LSOA21CD, sep="-"))
cbtmean <- crossbasis(data_adm_cardresp_sen_cts$tasmean, lag=14, argvar=cb_temp, arglag=cb_lag, group=group)

summary(cbtmean) # summary of cross-basis

# Define the strata - LSOA, year, month 
data_adm_cardresp_sen_cts[, stratum:=factor(paste(LSOA21CD, year, month, sep=":"))] # to compare days within the same LSOA and same month and year

# Run the model, including empty strata (otherwise bias in gnm with quasipoisson)
data_adm_cardresp_sen_cts[,  keep:=sum(total)>0, by=stratum]
...
# Re-define cross-basis, similar approach - no group
cbtmeanaggr <- crossbasis(data_adm_cardresp_sen_aggr$tasmean, lag=14, argvar=cb_temp, arglag=cb_lag)
...
# Save the plot
dev.print(svg, file="Outputs/Admissions/Cardresp cause/Sensitivity analysis/3. Main model (DLNM)/fig13_cardresp_sen_ctsaggr_plotadm.svg", width=7, height=4)
...
cbint1 <- cbtmean * (data_adm_cardresp_sen_cts$imdscore - intval[1])
...
cbint2<- cbtmean * (data_adm_cardresp_sen_cts$imdscore - intval[2])
...
# Save the plot
dev.print(svg, file="Outputs/Admissions/Cardresp cause/Sensitivity analysis/3. Main model (DLNM)/fig14_bhamimdplotadmsen.svg", width=7, height=4)
...
cbint3 <- cbtmean * (data_adm_cardresp_sen_cts$indoorssubscore - intvalsubdomain[1])
...
# Define interaction cross-bases and create percentiles (high)
cbint4 <- cbtmean * (data_adm_cardresp_sen_cts$indoorssubscore - intvalsubdomain[2])
...
# Save the plot
dev.print(svg, file="Outputs/Admissions/Cardresp cause/Sensitivity analysis/3. Main model (DLNM)/fig15_bhamsimdsubdomainplotadmsen.svg", width=7, height=4)
