...
# Number of calendar years (9.75 - 2015 starts in April) * 7) - 1
# Natural spline as the standard approach
data_adm_cardresp_sen_aggr$dayseq <- seq(nrow(data_adm_cardresp_sen_aggr))
spline <- ns(data_adm_cardresp_sen_aggr$dayseq,df=67)

# Fit quasi-poisson regression model (quasi-poisson for overdispersion - variance > mean)
qpmodeladmsen1 <- glm(total ~ spline,data_adm_cardresp_sen_aggr,family=quasipoisson)
...
# Save the plot
# ggsave("Outputs/Admissions/Cardresp cause/Sensitivity analysis/2. Quasi-poisson model (before DLNM)/figX_splinemodel_seasonality_trends_main.svg", width=10, height=5)
...
# Save the plot
# ggsave("Outputs/Admissions/Cardresp cause/Sensitivity analysis/2. Quasi-poisson model (before DLNM)/figX_resvar1_main.svg", width=10, height=5)
...
data_adm_cardresp_sen_aggr$dow <- as.factor(data_adm_cardresp_sen_aggr$dow)

# Fit second quasi-poisson regression model and include dow
qpmodeladmsen2 <- glm(total ~ spline+factor(dow),data_adm_cardresp_sen_aggr,family=quasipoisson)
...
# Save the plot
# ggsave("Outputs/Admissions/Cardresp cause/Sensitivity analysis/2. Quasi-poisson model (before DLNM)/figX_spline+dow_main.svg", width=10, height=5)
...
# Save the plot
# ggsave("Outputs/Admissions/Cardresp cause/Sensitivity analysis/2. Quasi-poisson model (before DLNM)/figX_resvardow2_main.svg", width=10, height=5)
...
data_adm_cardresp_sen_aggr_tempgroup <- data_adm_cardresp_sen_aggr %>%
...
data_adm_cardresp_sen_aggr_tempgroup$temp_group <- relevel(data_adm_cardresp_sen_aggr_tempgroup$temp_group, ref = "15-19°C") # Apply relevel function

# Unadjusted model - only temperature (categorical) and total admissions
unadjmodeltg_cardrespadmsen <- glm(total ~ temp_group, data_adm_cardresp_sen_aggr_tempgroup,family = quasipoisson())

summary(unadjmodeltg_cardrespadmsen)

# Exponentiate to get RR from log function
effunadjmodeltg_cardrespadmsen <- ci.lin(unadjmodeltg_cardrespadmsen,
                                         subset= "temp_group",
                                         Exp=T)
effunadjmodeltg_cardrespadmsen # view

# Adjusted model (1) to include spline (seasonality and long term trends)
adjmodeltg_cardrespadmsen1 <- glm(total ~ temp_group + spline,data_adm_cardresp_sen_aggr_tempgroup,family = quasipoisson())

summary(adjmodeltg_cardrespadmsen1)

# Exponentiate to get RR from log function
effadjmodeltg_cardrespadmsen1 <- ci.lin(adjmodeltg_cardrespadmsen1,
                                        subset= "temp_group",
                                        Exp=T)
effadjmodeltg_cardrespadmsen1 # view

# Adjusted model (2) to include spline + day of week

adjmodeltg_cardrespadmsen2 <- glm(
  total ~ temp_group + spline + dow,
  data_adm_cardresp_sen_aggr_tempgroup,
  family = quasipoisson())

summary(adjmodeltg_cardrespadmsen2)

# Exponentiate to get RR from log function
effadjmodeltg_cardrespadmsen2 <- ci.lin(adjmodeltg_cardrespadmsen2, subset= "temp_group", Exp=T)
effadjmodeltg_cardrespadmsen2 # view

# Adjusted model (3) to include spline + day of week + PM2.5

adjmodeltg_cardrespadmsen3 <- glm(
  total ~ temp_group + spline + dow + pm25,
  data_adm_cardresp_sen_aggr_tempgroup,
  family = quasipoisson())

summary(adjmodeltg_cardrespadmsen3)

# Exponentiate to get RR from log function
effadjmodeltg_cardrespadmsen3 <- ci.lin(adjmodeltg_cardrespadmsen3,
                                        subset= "temp_group",
                                        Exp=T)
effadjmodeltg_cardrespadmsen3 # view

effunadjmodeltg_cardrespadmsen # unadjusted
effadjmodeltg_cardrespadmsen1 # + seasonality and long term trends
effadjmodeltg_cardrespadmsen2 # dow
effadjmodeltg_cardrespadmsen3 # pm2.5

tabadjmodeltg_cardrespadmsen <- adjmodeltg_cardrespadmsen3 %>% 
  broom::tidy(exponentiate = TRUE, conf.int = TRUE) %>%  ## get a tidy dataframe of estimates 
  mutate(across(where(is.numeric), round, digits = 2))          ## round to 2 decimal places

tabadjmodeltg_cardrespadmsen

# Summary table (temperature as categorical variable) showing relative risks compared with the reference category
tabefftg_cardrespadmsen <-rbind(round(effunadjmodeltg_cardrespadmsen,2),round(effadjmodeltg_cardrespadmsen1,2),round(effadjmodeltg_cardrespadmsen2,2),round(effadjmodeltg_cardrespadmsen3,2)) [,5:7] # stacks three models and uses CI columns

dimnames(tabefftg_cardrespadmsen) <-list(c("Unadjusted <0°C",
                                           "Unadjusted 0-4°C",
                                           "Unadjusted 5-9°C",
                                           "Unadjusted 10-14°C",
                                           "Unadjusted >20°C",
...
colnames(tabefftg_cardrespadmsen) <- c("Relative Risk","Lower Confidence Interval","Upper Confidence Interval") # column names

tabefftg_cardrespadmsen ## View

# Forest plot to display regression results - model which adjusts for seasonality, long term trends and day of week + pm2.5

tabadjmodeltg_cardrespadmsen

tabadjmodeltg_cardrespadmsen %>% # remove intercept term from the multi-variable results
...
ggsave("Outputs/Admissions/Cardresp cause/Sensitivity analysis/2. Quasi-poisson model (before DLNM)/fig10_forestplotqpregresstotal_sen.svg", width=10, height=5)

# Model check using a scatter plot of deviance residuals vs time 

# Deviance residuals (previously it was response residuals)
devres_adm <- residuals(adjmodeltg_cardrespadmsen3,type="deviance")

#devres_admplot <- plot(data_adm_cardresp_sen_aggr_tempgroup$date,devres_adm,ylim=c(-5,10),pch=19,cex=0.7,col=grey(0.6),
#                   main="Deviance residuals from main model over time",ylab="Deviance residuals",xlab="Date")
# abline(h=0,lty=2,lwd=2)

# Save/print plot
# dev.print(svg, file="Outputs/Admissions/Cardresp cause/Sensitivity analysis/2. Quasi-poisson model (before DLNM)/figX_devrianceesidualsplot_main.svg", width=10, height=5)
...
# Save/print plot
# dev.print(svg, file="Outputs/Admissions/Cardresp cause/Sensitivity analysis/2. Quasi-poisson model (before DLNM)/figX_partialautocorrelationplot_main.svg", width=10, height=5)

rm(adjmodeltg_cardrespadmsen1,adjmodeltg_cardrespadmsen2,adjmodeltg_cardrespadmsen3, unadjmodeltg_cardrespadmsen, effadjmodeltg_cardrespadmsen1,effadjmodeltg_cardrespadmsen2,effadjmodeltg_cardrespadmsen3, effunadjmodeltg_cardrespadmsen, qpmodeladmsen1,qpmodeladmsen2,tabefftg_cardrespadmsen,tabadjmodeltg_cardrespadmsen) # remove data tables not required
