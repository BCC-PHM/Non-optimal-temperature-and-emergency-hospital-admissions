...
splinedayofyear <- onebasis(data_adm_cardresp_main_cts$doy, "ns", df=3) # to capture within year seasonal trends
...
cb_temp <- list(fun="ns", knots=quantile(data_adm_cardresp_main_cts$tasmean, c(10,50,90)/100, na.rm=TRUE))
cb_lag <- list(fun="ns", df=5)
group <- factor(paste(data_adm_cardresp_main_cts$LSOA21CD, data_adm_cardresp_main_cts$period, sep="-"))
cbtmean <- crossbasis(data_adm_cardresp_main_cts$tasmean, lag=14, argvar=cb_temp, arglag=cb_lag, group=group)

summary(cbtmean) # summary of cross-basis

# Define the strata - LSOA, year, month 
data_adm_cardresp_main_cts[, stratum:=factor(paste(LSOA21CD, year, month, sep=":"))] # to compare days within the same LSOA and same month and year

# Run the model, including empty strata (otherwise bias in gnm with quasipoisson)
data_adm_cardresp_main_cts[,  keep:=sum(total)>0, by=stratum]
...
cbtmeanaggr <- crossbasis(data_adm_cardresp_main_aggr$tasmean, lag=14, argvar=cb_temp, arglag=cb_lag,
                          group=data_adm_cardresp_main_aggr$period)
...
fullmainplot <- plot(cpfull_adm, "overall", ylim=c(0.8,1.5), ylab="RR", col=col[1], lwd=1.5,
...
# Save the plot
dev.print(svg, file="Outputs/Admissions/Cardresp cause/Main/3. Main model (DLNM)/fig28_cardresp_main_ctsaggr_plotadm.svg", width=7, height=4)
...
datesusedinmodel <- data_adm_cardresp_main_cts$date[data_adm_cardresp_main_cts$keep]
...
# Save/print plot
dev.copy(png, file="Outputs/Admissions/Cardresp cause/Main/3. Main model (DLNM)/fig29_devrianceesidualsplot_main.png")
dev.off()
...
# Save/print plot
dev.print(svg, file="Outputs/Admissions/Cardresp cause/Main/3. Main model (DLNM)/fig30_partialautocorrelationplot_main.svg", width=10, height=5)
...
# Join by LSOA
data_adm_cardresp_main_cts <- data_adm_cardresp_main_cts %>%
  left_join( bhamlsoa21imd) # Joining with `by = join_by(LSOA21)

rm(bhamlsoaimd,bhamlsoashp)

head(data_adm_cardresp_main_cts) # check data frame
...
cbint1 <- cbtmean * (data_adm_cardresp_main_cts$imdscore - intval[1])
...
cbint2<- cbtmean * (data_adm_cardresp_main_cts$imdscore - intval[2])
...
# Save the plot
dev.print(svg, file="Outputs/Admissions/Cardresp cause/Main/3. Main model (DLNM)/fig31_bhamimdplotcardrespadmmain.svg", width=7, height=4)
...
cbint3 <- cbtmean * (data_adm_cardresp_main_cts$indoorssubscore - intvalsubdomain[1])
...
# Define interaction cross-bases and create percentiles (high)
cbint4 <- cbtmean * (data_adm_cardresp_main_cts$indoorssubscore - intvalsubdomain[2])
...
# Save the plot
dev.print(svg, file="Outputs/Admissions/Cardresp cause/Main/3. Main model (DLNM)/fig32_bhamsimdsubdomainplotcardrespadmmain.svg", width=7, height=4)
