...
##---- Step 2.0: Prepare time series data - All cause admissions - Main analysis (excluding Covid-19 pandemic)

# 2.1: Load admissions data, format and sort

# Load data from SQL Server output (csv)
data_adm_allcause_main <- as.data.table(read.csv("Data/Outcomes/admissions/bhamlsoaadmmain.csv"))

# Check missing data
colSums(is.na(data_adm_allcause_main))

# Rename columns
names(data_adm_allcause_main) <- c("year",
                                   "month",
                                   "day",
                                   "LSOA11CD",
                                   "a04",
...
data_adm_allcause_main[, date:=as.Date(paste(year, month, day, sep="/"))]

# Define series of unique LSOAs and dates
seqlsoa <- sort(unique(data_adm_allcause_main$LSOA11CD)) # sorted list of unique LSOA codes
seqdate <- sort(unique(data_adm_allcause_main$date)) # sorted list of unique dates
table(diff(seqdate)) # checking gaps between consecutive dates - should be 1 day gaps

# Create a continuous sequence of dates
seqdate <- seq(min(data_adm_allcause_main$date), # first date in dataset
               max(data_adm_allcause_main$date), # last date in dataset
...
data_adm_allcause_main[, .(n_lsoa = uniqueN(LSOA11CD)), by = year] # count unique LSOAs per year

data_adm_allcause_main[, uniqueN(LSOA11CD)] # check across the whole dataset - some missing from some years which may have had zero emergency admissions

# Develop full dataset including days with zero admissions
data_adm_allcause_main_cts <- expand.grid(LSOA11CD=seqlsoa, date=seqdate) |>
  data.table() |> # converts it to data table
  merge(data_adm_allcause_main, by = c("LSOA11CD", "date"), all.x=T) # merge  in admission counts and keep all rows from new dataset

# Replace N/A with 0 
data_adm_allcause_main_cts[is.na(data_adm_allcause_main_cts)] <- 0 # replace N/A with zero when no adm

# Re-create time variables including day of year and week
data_adm_allcause_main_cts[, `:=`(year=year(date),
                                  month=month(date),
                                  day=mday(date),
                                  doy=yday(date),
                                  dow=wday(date))]
...
data_adm_allcause_main <- data_adm_allcause_main %>%
  filter(!between(date, as.Date("2020-03-01"), as.Date("2021-04-30")))
...
data_adm_allcause_main_cts <- data_adm_allcause_main_cts %>% # Join data with the lookup
...
data_adm_allcause_main_cts <- as.data.table(data_adm_allcause_main_cts) %>%
...
setkey(data_adm_allcause_main_cts, LSOA21CD, date) # data.table command - sorts by LSOA21 and then by date
...
data_adm_allcause_main_aggr <- data_adm_allcause_main[, lapply(.SD, sum), by=date, # calculation by date
                                                      .SDcols=c("a04","a514","a1564", "a6574","a75plus","total")] # only use these columns for the calculation

# Re-create time variables including day of year and week
data_adm_allcause_main_aggr[, `:=`(year=year(date),
                                   month=month(date),
                                   day=mday(date),
                                   doy=yday(date),
...
# Sort by date
setkey(data_adm_allcause_main_aggr, date) # data.table command - sorts by date

#Remove files and data not required
rm(data_adm_allcause_main) # Original table no longer required
