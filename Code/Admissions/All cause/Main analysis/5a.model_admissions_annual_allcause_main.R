...
# Number of calendar years * 7, but
# Removal of Covid years for main analysis
# Natural spline as the standard approach
data_adm_allcause_main_aggr$dayseq <- seq(nrow(data_adm_allcause_main_aggr))
spline <- ns(data_adm_allcause_main_aggr$dayseq,df=59)

# Fit quasi-poisson regression model (quasi-poisson for overdispersion - variance > mean)
qpmodeladm1 <- glm(total ~ spline,data_adm_allcause_main_aggr,family=quasipoisson)
summary(qpmodeladm1)

# Predict number of admissions from qpmodeladm1
pred1 <- predict(qpmodeladm1,type="response") # expected admissions for each date in dataset
...
# Save the plot
ggsave("Outputs/Admissions/All cause/Main/2. Quasi-poisson model (before DLNM)/fig14_splinemodel_seasonality_trends_main.svg", width=10, height=5)

# Plot response residuals from model (observed - expected)
residuals <- residuals(qpmodeladm1,type="response")
ggplot(data_adm_allcause_main_aggr,
...
# Save the plot
ggsave("Outputs/Admissions/All cause/Main/2. Quasi-poisson model (before DLNM)/fig15_resvar1_main.svg", width=10, height=5)
...
data_adm_allcause_main_aggr$dow <- as.factor(data_adm_allcause_main_aggr$dow)

# Fit second quasi-poisson regression model and include dow
qpmodeladm2 <- glm(total ~ spline+factor(dow),data_adm_allcause_main_aggr,family=quasipoisson)
summary(qpmodeladm2)

# Predict number of admissions from qpmodeladm2
pred2 <- predict(qpmodeladm2,type="response") # expected admissions for each date in dataset
...
# Save the plot
ggsave("Outputs/Admissions/All cause/Main/2. Quasi-poisson model (before DLNM)/fig16_spline+dow_main.svg", width=10, height=5)

# Plot response residuals from model and generate residuals (observed - expected)
residuals <- residuals(qpmodeladm2,type="response")
ggplot(data_adm_allcause_main_aggr,
       aes(x=date,residuals))+
  geom_point(
    colour = "black",
...
# Save the plot
ggsave("Outputs/Admissions/All cause/Main/2. Quasi-poisson model (before DLNM)/fig17_resvardow2_main.svg", width=10, height=5)
...
data_adm_allcause_main_aggr_tempgroup <- data_adm_allcause_main_aggr %>%
...
data_adm_allcause_main_aggr_tempgroup$temp_group <- relevel(data_adm_allcause_main_aggr_tempgroup$temp_group, ref = "15-19°C") # Apply relevel function

# Unadjusted model - only temperature (categorical) and total admissions
unadjmodeltg_adm <- glm(total ~ temp_group, data_adm_allcause_main_aggr_tempgroup,family = quasipoisson())

summary(unadjmodeltg_adm)

# Exponentiate to get RR from log function
effunadjmodeltg_adm <- ci.lin(unadjmodeltg_adm,
                          subset= "temp_group",
                          Exp=T)
effunadjmodeltg_adm # view

# Adjusted model (1) to include spline (seasonality and long term trends)
adjmodeltg_adm1 <- glm(total ~ temp_group + spline,data_adm_allcause_main_aggr_tempgroup,family = quasipoisson())

summary(adjmodeltg_adm1)

# Exponentiate to get RR from log function
effadjmodeltg_adm1 <- ci.lin(adjmodeltg_adm1,
                         subset= "temp_group",
                         Exp=T)
effadjmodeltg_adm1 # view

# Adjusted model (2) to include spline + day of week

adjmodeltg_adm2 <- glm(
  total ~ temp_group + spline + dow,
  data_adm_allcause_main_aggr_tempgroup,
  family = quasipoisson())

summary(adjmodeltg_adm2)

# Exponentiate to get RR from log function
effadjmodeltg_adm2 <- ci.lin(adjmodeltg_adm2, subset= "temp_group", Exp=T)
effadjmodeltg_adm2 # view
...
summary(adjmodeltg_adm3)

# Exponentiate to get RR from log function
effadjmodeltg_adm3 <- ci.lin(adjmodeltg_adm3,
                         subset= "temp_group",
                         Exp=T)
effadjmodeltg_adm3 # view

effunadjmodeltg_adm # unadjusted
effadjmodeltg_adm1 # + seasonality and long term trends
effadjmodeltg_adm2 # dow
effadjmodeltg_adm3 # pm2.5

tabadjmodeltg_adm <- adjmodeltg_adm3 %>% 
...
tabefftg_adm <-rbind(round(effunadjmodeltg_adm,2),round(effadjmodeltg_adm1,2),round(effadjmodeltg_adm2,2),round(effadjmodeltg_adm3,2)) [,5:7] # stacks three models and uses CI columns
...
tabadjmodeltg_adm %>% # remove intercept term from the multi-variable results
...
ggsave("Outputs/Admissions/All cause/Main/2. Quasi-poisson model (before DLNM)/fig18_forestplotqpregresstotal_main.svg", width=10, height=5)

# Model check using a scatter plot of deviance residuals vs time 

# Deviance residuals (previously it was response residuals)
devres_adm <- residuals(adjmodeltg_adm3,type="deviance")

devres_admplot <- plot(data_adm_allcause_main_aggr_tempgroup$date,devres_adm,ylim=c(-5,10),pch=19,cex=0.7,col=grey(0.6),
                   main="Deviance residuals from main model over time",ylab="Deviance residuals",xlab="Date")
abline(h=0,lty=2,lwd=2)

# Save/print plot
dev.print(svg, file="Outputs/Admissions/All cause/Main/2. Quasi-poisson model (before DLNM)/fig19_devrianceesidualsplot_main.svg", width=10, height=5)
...
# Save/print plot
dev.print(svg, file="Outputs/Admissions/All cause/Main/2. Quasi-poisson model (before DLNM)/fig20_partialautocorrelationplot_main.svg", width=10, height=5)

rm(adjmodeltg_adm1,adjmodeltg_adm2,adjmodeltg_adm3, unadjmodeltg_adm, effadjmodeltg_adm1,effadjmodeltg_adm2,effadjmodeltg_adm3, effunadjmodeltg_adm, qpmodeladm1,qpmodeladm2,tabefftg_adm,tabadjmodeltg_adm) # remove data tables not required
