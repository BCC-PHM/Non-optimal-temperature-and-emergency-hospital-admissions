...
splinedayofyear <- onebasis(data_adm_allcause_main_cts$doy, "ns", df=3) # to capture within year seasonal trends
...
cb_temp <- list(fun="ns", knots=quantile(data_adm_allcause_main_cts$tasmean, c(10,50,90)/100, na.rm=TRUE))
cb_lag <- list(fun="ns", df=5)
group <- factor(paste(data_adm_allcause_main_cts$LSOA21CD, data_adm_allcause_main_cts$period, sep="-")) # period instead of year to ensure continuation until Covid
cbtmean <- crossbasis(data_adm_allcause_main_cts$tasmean, lag=14, argvar=cb_temp, arglag=cb_lag, group=group)

summary(cbtmean) # summary of cross-basis

# Define the strata - LSOA, year, month 
data_adm_allcause_main_cts[, stratum:=factor(paste(LSOA21CD, year, month, sep=":"))] # to compare days within the same LSOA and same month and year

# Run the model, including empty strata (otherwise bias in gnm with quasipoisson)
data_adm_allcause_main_cts[,  keep:=sum(total)>0, by=stratum]
...
cbtmeanaggr <- crossbasis(data_adm_allcause_main_aggr$tasmean, lag=14, argvar=cb_temp, arglag=cb_lag,
                          group=data_adm_allcause_main_aggr$period)
...
fullmainplot <- plot(cpfull_adm, "overall", ylim=c(0.8,1.2), ylab="RR", col=col[1], lwd=1.5,
...
# Add legend to plot
legend("top", c("Case Time Series", "Aggregated Time Series"), lty=1, lwd=1.5, col=col, bty="n",
       inset=0.05, y.intersp=2, cex=0.8)
par(parold) # restore original plot settings

# Save the plot
dev.print(svg, file="Outputs/Admissions/All cause/Main/3. Main model (DLNM)/fig31_allcause_main_ctsaggr_plotadm.svg", width=7, height=4)

# Model check using a scatter plot of deviance residuals vs time

# Deviance residuals (previously it was response residuals)
devres_adm_full <- residuals(full_model_adm,type="deviance")

# Only use dates actually used in the model
datesusedinmodel <- data_adm_allcause_main_cts$date[data_adm_allcause_main_cts$keep]
datesusedinmodel <- datesusedinmodel[-full_model_adm$na.action]

devres_adm_fullplot <- plot(datesusedinmodel,devres_adm_full,ylim=c(-5,10),pch=19,cex=0.7,col=grey(0.6), #removing dates that were removed from final model (e.g. start of year without lag dates)
                       main="Deviance residuals from main model over time",ylab="Deviance residuals",xlab="Date")
abline(h=0,lty=2,lwd=2)

# Save/print plot
dev.copy(png, file="Outputs/Admissions/All cause/Main/3. Main model (DLNM)/fig32_devrianceesidualsplot_main.png")
dev.off()

# Partial autocorrelation function to measure degree of autocorrelation between days
pacf(devres_adm_full,na.action=na.omit,main="Partial autocorrelation plot of deviance residuals")

# Save/print plot
dev.print(svg, file="Outputs/Admissions/All cause/Main/3. Main model (DLNM)/fig33_partialautocorrelationplot_main.svg", width=10, height=5)
...
head(data_adm_allcause_main_cts) # check data frame
...
cbint1 <- cbtmean * (data_adm_allcause_main_cts$imdscore - intval[1])
...
cbint2<- cbtmean * (data_adm_allcause_main_cts$imdscore - intval[2])
...
# Save the plot
dev.print(svg, file="Outputs/Admissions/All cause/Main/3. Main model (DLNM)/fig34_bhamimdplotadm.svg", width=7, height=4)
...
cbint3 <- cbtmean * (data_adm_allcause_main_cts$indoorssubscore - intvalsubdomain[1])
...
# Define interaction cross-bases and create percentiles (high)
cbint4 <- cbtmean * (data_adm_allcause_main_cts$indoorssubscore - intvalsubdomain[2])
...
# Save the plot
dev.print(svg, file="Outputs/Admissions/All cause/Main/3. Main model (DLNM)/fig35_bhamsimdsubdomainplotadm.svg", width=7, height=4)
